...
class Kinematics {
  public:
    ...
    // Quaternion Matrix Calculation
    Matrix4d quat_mat(const Vector4d& q) {
    /**
     * @brief Constructs a quaternion matrix from a quaternion.
     *
     * This function constructs a 4x4 quaternion matrix from the given
     * quaternion. The quaternion is represented as a 4D vector.
     *
     * @param q The quaternion (Eigen::Vector4d).
     * @return A 4x4 Eigen::Matrix representing the quaternion matrix.
     */
        Matrix4d q_mat;
        q_mat << q(0), -q(1), -q(2), -q(3),
                 q(1),  q(0), -q(3),  q(2),
                 q(2),  q(3),  q(0), -q(1),
                 q(3), -q(2),  q(1),  q(0);
        return q_mat;
